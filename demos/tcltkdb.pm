...
# - remove many of obsolete code: get_notebook_widget, register_user_DB_entry, 
#   register_user_window_init, setTabs, etc,
#   code that eval's chunks in ptkdbrc file to support changing colors of text widget, etc.
# - removed all code about configurung scrollbars of tcltkdb
# - removed $Devel::tcltkdb::add_expr_depth; # how much further to expand an expression when clicked
# - removed $Devel::tcltkdb::expr_depth = -1 ;
# - $self->{DisableOnLeave} neverused, remove (List of Widgets to disable when leaving the debugger)
